...
class DBConnection{
    ...
    function linkaccalready($accNumber,$jwt,$cif,$tblLinkAcc){
        $databaseService = new DatabaseService();
        $conn = $databaseService->getConnection();

        $query_finish="SELECT * FROM `$tblLinkAcc` WHERE `account`='$accNumber' AND `token`='$jwt' AND `cif`='$cif' ";
        $cmd_finish=$conn->prepare($query_finish);
        $cmd_finish->execute();
    ...
    function insert_intit_transaction($refNumber,$type,$sourceAcc,$destinationAcc,$amount,$ccy,$fee,$desc,$tblTrx){
        ...
        $query_trx="INSERT INTO `$tblTrx` (`refNumber` , `transType`, `fromAcc`, `toAcc`, `amount`, `currency`, `fee` , `dbtCdt`, `desc`, `status`,`datetimeinmilisecond`, `transDate`) 
                    VALUES ('$refNumber','$type','$sourceAcc','$destinationAcc','$amount','$ccy','$fee','Debit','$desc','PENDING','$milliseconds','$transDate')";
    ...
    function inquiry_transaction($initRefNumber,$tblTrx){
        $databaseService = new DatabaseService();
        $conn = $databaseService->getConnection();
        $query_acc="SELECT * FROM `$tblTrx` WHERE `refNumber`='$initRefNumber'  AND `transDate` BETWEEN DATE_SUB(CURDATE(), INTERVAL 1 DAY) AND DATE_ADD(CURDATE(), INTERVAL 1 DAY) ";
    ...
    function update_finish_transaction($initRefNumber,$transId,$transHash,$milliseconds,$transDate,$tblTrx){
        $databaseService = new DatabaseService();
        $conn = $databaseService->getConnection();

        $query_finTrx="UPDATE `$tblTrx` SET `transId`= '$transId' , `transHash`= '$transHash' , `datetimeinmilisecond`='$milliseconds',`transDate`='$transDate', `status`='SUCCESS' WHERE `refNumber`='$initRefNumber' ";
    ...
    function getAccountTransction($accNumber,$size,$tblTrx){
        $databaseService = new DatabaseService();
        $conn = $databaseService->getConnection();
        $query_trans="SELECT * FROM `$tblTrx` WHERE `fromAcc`='$accNumber' AND `status` = 'SUCCESS' LIMIT $size";

        $cmd_trans=$conn->prepare($query_trans);
        $cmd_trans->execute();
